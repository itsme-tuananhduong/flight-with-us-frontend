...
const Ticket = ({
  ...
  const NgayKhoiHanh = new Date(ThoiGianKhoiHanh).toLocaleDateString();
  const NgayHaCanh = new Date(ThoiGianHaCanh).toLocaleDateString();
  const GioKhoiHanh = ThoiGianKhoiHanh.substr(16, 5);
  const GioHaCanh = ThoiGianHaCanh.substr(16, 5);
  const ThoiGianDiChuyen = timeDiffCalc(
    new Date(ThoiGianHaCanh),
    new Date(ThoiGianKhoiHanh)
  );

  GiaVe = parseFloat(GiaVe);
  Thue = parseFloat(Thue);
  ...
  useEffect(() => {
    if (choosenTicketInfo.IdVeMayBay === null) {
      // setIdVe(IdVeMayBay);
      setIsChoosen(false);
    } else if (choosenTicketInfo.IdVeMayBay === IdVeMayBay) {
      // setIdVe(null);
      setIsChoosen(true);
    } else if (choosenTicketInfo.IdVeMayBay !== IdVeMayBay) {
      // setIdVe(null);
  ...
  }, [choosenTicketInfo.IdVeMayBay]);

  const onTicketChoose = () => {
    if (choosenTicketInfo.IdVeMayBay === null) {
      setChoosenTicketInfo({
    ...
    >
      <div className="ticket">
        <div className="ticket-logo-img">
          <img src={imgLink} alt="ticket-logo-img" />
        </div>
        <div className="ticket-body">
          ...
          <div className="ticket-body__box-3">
            <div className="depature-time">
              <span className="time-span">{NgayKhoiHanh}</span>
              <p className="time">{GioKhoiHanh}</p>
              <span className="time-span">{DiaDiemKhoiHanh}</span>
            </div>
            <div className="flight-flying-time">
              <span className="flying-time">{ThoiGianDiChuyen}</span>
              <img src={flightTypeImg} alt="airplane" />
              <span className="flying-type">{LoaiHinhBay}</span>
            </div>
            <div className="landing-time">
              <span className="time-span">{NgayHaCanh}</span>
              <p className="time">{GioHaCanh}</p>
              <span className="time-span">{DiaDiemHaCanh}</span>
            </div>
          </div>
          ...
          <div className="ticket-body__box-5">
            <b>{moneyFormatter.format(GiaVe)}</b>
            {isChoosen && (
              <button className="select-btn choosen" onClick={onTicketChoose}>
                Bỏ chọn
              </button>
            )}
            {!isChoosen && (
              <button className="select-btn" onClick={onTicketChoose}>
                Chọn
              </button>
            )}
          </div>
      ...
      >
        <div className="ticket-detail__main">
          <div className="flight-detail">
            ...
            <div className="flight-detail__box-3">{ThoiGianDiChuyen}</div>
            <div className="flight-detail__box-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
          ...
          <div className="price-detail">
            ...
            <div className="price-detail__box-wrapper">
              ...
              {passengers.adult !== 0 && (
                <div className="passenger">
                  ...
                  <span>{moneyFormatter.format(passengers.adult * GiaVe)}</span>
                  <span>
                    {moneyFormatter.format(
                      passengers.adult * GiaVe * (Thue / 100)
                    )}
                  </span>
                  <span>
                    {moneyFormatter.format(
                      passengers.adult * (GiaVe + GiaVe * (Thue / 100))
                    )}
                  </span>
                </div>
              )}
              {passengers.child !== 0 && (
                <div className="passenger">
                  ...
                  <span>{moneyFormatter.format(passengers.child * GiaVe)}</span>
                  <span>
                    {moneyFormatter.format(
                      passengers.child * GiaVe * (Thue / 100)
                    )}
                  </span>
                  <span>
                    {moneyFormatter.format(
                      passengers.child * (GiaVe + GiaVe * (Thue / 100))
                    )}
                  </span>
                </div>
              )}
              {passengers.baby !== 0 && (
                <div className="passenger">
                  ...
                  <span>{moneyFormatter.format(passengers.baby * GiaVe)}</span>
                  <span>
                    {moneyFormatter.format(
                      passengers.baby * GiaVe * (Thue / 100)
                    )}
                  </span>
                  <span>
                    {moneyFormatter.format(
                      passengers.baby * (GiaVe + GiaVe * (Thue / 100))
                    )}
                  </span>
                </div>
              )}
            ...
            <div className="price-detail__box-2">
              <b>
                Tổng tiền: <span>{moneyFormatter.format(tongTien)}</span>
              </b>
            </div>
          </div>
        </div>
