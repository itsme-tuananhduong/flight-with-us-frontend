...
const FlightInfoBox = () => {
  ...
  const [flightInfo, setFlightInfo] = useState({
    IdVeMayBay: searchParams.get('idvemaybay'),
    LoaiVe: searchParams.get('loaive'),
    GiaVe: parseFloat(searchParams.get('giave')),
    HangHK: searchParams.get('hanghk'),
    SHMayBay: searchParams.get('shmaybay'),
    ThoiGianKhoiHanh: searchParams.get('tgkh'),
    ThoiGianHaCanh: searchParams.get('tghc'),
    DiaDiemKhoiHanh: searchParams.get('ddkh'),
    DiaDiemHaCanh: searchParams.get('ddhc'),
    LoaiHinhBay: searchParams.get('loaihinhbay'),
    passengers: {
    ...
    Thue: parseInt(searchParams.get('thue')),
  });

  let flightTypeImg = '';

  if (flightInfo.LoaiHinhBay === 'Bay thẳng') {
    flightTypeImg =
  ...
  } else if (flightInfo.LoaiHinhBay === 'Quá cảnh') {
    flightTypeImg =
  ...
  if (flightInfo.HangHK === 'Vietnam Airlines')
    imgLink =
      'https://flight.hahalolo.com/e79fa58a4bf9d10176f38e36f1d157ce.svg';
  else if (flightInfo.HangHK === 'Vietravel Airlines')
    imgLink =
  ...
  else if (flightInfo.HangHK === 'Bamboo Airways')
    imgLink =
      'https://flight.hahalolo.com/bb8c9a3f56366b5c501f0db5574d0942.svg';
  else if (flightInfo.HangHK === 'Vietjet Air')
    imgLink =
    ...
    <div className={theme === 'dark' ? 'flight-info dark' : 'flight-info'}>
      ...
      <div className="flight-info__content">
        ...
        <div className="depature-day">
          <p>
            {new Date(flightInfo.ThoiGianKhoiHanh).toString().substr(0, 15)}
          </p>
        </div>
        <div className="flight-passenger">
          <p>
            Hành khách:{' '}
            <span>
              {flightInfo.passengers.adult !== 0 ? (
                <span>&nbsp;{flightInfo.passengers.adult} người lớn</span>
              ) : null}
              {flightInfo.passengers.child !== 0 ? (
                <span>,&nbsp;{flightInfo.passengers.child} trẻ em</span>
              ) : null}
              {flightInfo.passengers.baby !== 0 ? (
                <span>,&nbsp;{flightInfo.passengers.baby} em bé</span>
              ) : null}
            </span>
          </p>
        </div>
        <div className="time-group">
          <div className="depature-time">
            <p className="time">{flightInfo.ThoiGianKhoiHanh.substr(16, 5)}</p>
            <p className="place">{flightInfo.DiaDiemKhoiHanh}</p>
          </div>
          <div className="flight-flying-time">
            <span className="flying-time">
              {timeDiffCalc(
                new Date(flightInfo.ThoiGianHaCanh),
                new Date(flightInfo.ThoiGianKhoiHanh)
              )}
            </span>
            <img src={flightTypeImg} alt="airplane" />
            <span className="flying-type">{flightInfo.LoaiHinhBay}</span>
          </div>
          <div className="landing-time">
            <p className="time">{flightInfo.ThoiGianHaCanh.substr(16, 5)}</p>
            <p className="place">{flightInfo.DiaDiemHaCanh}</p>
          </div>
        </div>
          ...
          <p>
            ...
            <span>
              {moneyFormatter.format(
                (flightInfo.passengers.adult +
                  flightInfo.passengers.child +
                  flightInfo.passengers.baby) *
                  (flightInfo.GiaVe +
                    flightInfo.GiaVe * (flightInfo.Thue / 100))
              )}
            </span>
          </p>
        ...
        >
          ...
          {flightInfo.passengers.adult !== 0 && (
            <div className="price-item">
              <span>
                Người lớn <b>x{flightInfo.passengers.adult}</b>
              </span>
              <b>
                {moneyFormatter.format(
                  flightInfo.passengers.adult * flightInfo.GiaVe
                )}
              </b>
            </div>
          )}
          ...
          {flightInfo.passengers.child !== 0 && (
            <div className="price-item">
              <span>
                Trẻ em <b>x{flightInfo.passengers.child}</b>
              </span>
              <b>
                {moneyFormatter.format(
                  flightInfo.passengers.child * flightInfo.GiaVe
                )}
              </b>
            </div>
          )}
          ...
          {flightInfo.passengers.baby !== 0 && (
            <div className="price-item">
              <span>
                Em bé <b>x{flightInfo.passengers.baby}</b>
              </span>
              <b>
                {moneyFormatter.format(
                  flightInfo.passengers.baby * flightInfo.GiaVe
                )}
              </b>
            </div>
          )}
